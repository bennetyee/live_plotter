...
use egui_plot::{Corner, GridInput, GridMark, Line, Plot, PlotBounds, PlotPoints};
...
// Optimized Time-Domain M4 Decimation
fn m4(
    buf: &SeriesBuffer,
    start: usize,
    end: usize,
    ...
    let inv_dx = if dx > 0.0 && !dx.is_nan() {
        1.0 / dx
    } else {
        0.0
    };
...
struct LivePlotApp {
    ...
    identifying: bool,
    identified_point: Option<(usize, f64, f64)>,
    highlighted_series: Option<usize>,
}
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                // Fixed 250.0 width locks the slider to prevent jitter from elements on the right
                if ui
                    .add_sized(
                        [250.0, 20.0],
                ...
                // DATA IDENTIFICATION TOOL UI
                let mut toggle_ident = self.identifying;
                let btn_label = if toggle_ident {
                    "Cancel Point ID"
                } else {
                    "Identify Point"
                };

                if ui.toggle_value(&mut toggle_ident, btn_label).clicked() {
                    self.identifying = toggle_ident;
                    if self.identifying {
                        self.identified_point = None;
                    }
                }

                if let Some((si, x, y)) = self.identified_point {
                    let xf = format_x_val(x, self.is_ts_mode);
                    ui.label(
            ...
            ui.with_layout(body_layout, |ui| {
                ...
                // Initialize standard plot without built-in Legend
                ...
                let plot_res = plot.show(ui, |plot_ui| {
                    ...
                    for (i, pts) in self.cached_lines.iter().enumerate() {
                        ...
                        let mut line_color = colors[i];
                        let mut line_width = 1.0;

                        // Apply Selected Series Highlight
                        if let Some(hs) = self.highlighted_series {
                            if hs == i {
                                line_width = 2.5;
                            } else {
                                line_color = line_color.linear_multiply(0.25);
                            }
                        }
                    ...
                    // Draw identified specific data point
                    if let Some((si, x, y)) = self.identified_point {
                        if visible[si] {
                            plot_ui.points(
                ...
                // DATA IDENTIFICATION SEARCH LOGIC
                // Only executes one time upon primary click during identifying mode
                if self.identifying && plot_res.response.clicked() {
                    if let (Some(mp), Some(b)) = (click_mp, current_bounds) {
                        let d = data_arc.lock().unwrap();
                        let mut best = None;
                        let mut bd = f64::INFINITY;
                        for si in 0..self.labels.len() {
                            if !self.visible[si] {
                                continue;
                            }
                        ...
                        if let Some((si, x, y)) = best {
                            if bd < (b.width() * 0.05).powi(2) {
                                self.identified_point = Some((si, x, y));
                            }
                        }
                        self.identifying = false; // Turn tool off after clicking
                    }
                }

                if self.identifying && plot_res.response.hovered() {
                ...
                // CUSTOM LEGEND OVERLAY (Replaces heavy egui_plot builtin)
                        ...
                        .show(ctx, |ui| {
                            ...
                            for i in 0..self.labels.len() {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    let is_highlighted = self.highlighted_series == Some(i);
                                    let text_color = if is_highlighted {
                                        Color32::WHITE
                                    } else if self.highlighted_series.is_some() {
                                        Color32::GRAY
                                    } else {
                            ...
                            // Clear highlight if clicked on empty legend background
                            ...
                            if let Some(i) = item_clicked {
                                self.highlighted_series = Some(i);
                            }
                        });
                }
            });
